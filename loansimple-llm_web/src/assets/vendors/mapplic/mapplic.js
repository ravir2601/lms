...
;(function($) {
	...
	var Mapplic = function(element) {
		...
		function Sidebar() {
			...
			this.addLocation = function(data) {
				var item = $('<li></li>').addClass('mapplic-list-location').addClass('mapplic-application-list-shown').attr('data-location', data.id);
				...
				var category = $('.mapplic-application-list-category[data-category="' + data.category + '"]');

				if (category.length) $('ol', category).append(item);
				else this.list.append(item);

				// Count
				$('.mapplic-application-list-count', category).text($('.mapplic-application-list-shown', category).length);
			}
			...
			this.search = function(keyword) {
				...
				$('.mapplic-application-list li', self.el).each(function() {
					if ($(this).text().search(new RegExp(keyword, "i")) < 0) {
						$(this).removeClass('mapplic-application-list-shown');
						$(this).slideUp(200);
					} else {
						$(this).addClass('mapplic-application-list-shown');
						$(this).show();
					}
				});

				$('.mapplic-application-list > li', self.el).each(function() {
					var count = $('.mapplic-application-list-shown', this).length;
					$('.mapplic-application-list-count', this).text(count);
				});

				// Show not-found text
				if ($('.mapplic-application-list > li.mapplic-application-list-shown').length > 0) this.notfound.fadeOut(200);
	...
	// For the full application-list of easing functions use jQuery Easing Plugin
	$.extend($.easing,
	{
		def: 'easeOutQuad',
